...
function game_createEnemy() {
    if(game_arrEnemy.length < 20) {
        ...
        game_arrEnemy.push({x: game_tx, y: game_ty, wh: game_twh, c: game_tc, v: game_tv})
    }
}

function game_drawEnemy() {
    for(var i = 0; i < game_arrEnemy.length; i++) {
        ctx.fillStyle = game_arrEnemy[i].c;
        ctx.fillRect(game_arrEnemy[i].x, game_arrEnemy[i].y, game_arrEnemy[i].wh, game_arrEnemy[i].wh);
    }
}

function game_updateEnemy() {
    for(var i = 0; i < game_arrEnemy.length; i++) {
        game_arrEnemy[i].x -= game_arrEnemy[i].v;
    }
}

function game_deleteEnemy() {
    for(var i = 0; i < game_arrEnemy.length; i++) {
        if(game_arrEnemy[i].x + game_arrEnemy[i].wh < 0) {
            game_arrEnemy.splice(i, 1);
        }
    }
}
...
function game_gameInfo() {
    ctx.font = "20px Georgio";
    ctx.fillStyle = "white";
    ctx.fillText("type: " + game_stype + "  Rocket: " + game_arrRocket.length + "  Enemy: " + game_arrEnemy.length, 10 ,20);
}
...
function init() {
    ...
    setInterval(game_createEnemy, 500);
