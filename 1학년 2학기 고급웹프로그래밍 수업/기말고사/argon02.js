...
// ---- 추가코드 시작 -----
var tu_bb; // 바운딩 박스 버튼
var tu_chR = true; // 비행선 방향 바꾸는 변수
var tu_chE; // 적 방향 바꾸기 버튼
...
function tu_colisionShip() {
  ...
  var tu_rt6 = {}, tu_rt7 = {}, tu_rt8 = {}, tu_rt9 = {};
  ...
  tu_rt6 = { x: sx, y: sy - 10, w: 20, h: 20 };
  tu_rt7 = { x: sx + 40, y: sy - 5, w: 20, h: 10 };
  tu_rt8 = { x: sx + 10, y: sy - 15, w: 10, h: 30 };
  tu_rt9 = { x: sx, y: sy - 20, w: 10, h: 40 };

  for (var i = 0; i < arrEnemy.length; i++) {
    tu_flag = false;
    tu_rt = { x: arrEnemy[i].x, y: arrEnemy[i].y, w: arrEnemy[i].w, h: arrEnemy[i].h };

    if (tu_chR) {                          // 우측 바라보는 비행선 콜라이더 작동
      if (tu_hitRectToRect(tu_rt, tu_rt6)) {
        tu_flag = true;
      }
      if (tu_hitRectToRect(tu_rt, tu_rt7) && stype === 1) {
        tu_flag = true;
      }
      if (tu_hitRectToRect(tu_rt, tu_rt8) && stype > 1) {
        tu_flag = true;
      }
      if (tu_hitRectToRect(tu_rt, tu_rt9) && stype > 2) {
        tu_flag = true;
      }
    } else {                                 // 좌측 바라보는 비행선 콜라이더 작동
...
//Bonding Box
function tu_drawBox() {
  if (tu_bb) {                  // 오른쪽 방향 비행선 콜라이더
    if (tu_chR) {
      ctx.strokeStyle = "white";
      ctx.strokeRect(sx, sy - 10, 20, 20);
    ...
    ctx.fillStyle = "white";
    ctx.fillRect(300, 0, 1, vcanvas.height);
    ctx.fillRect(600, 0, 1, vcanvas.height);
  }
}
// ---- 추가코드 끝 -----
...
function createEnemy() {
  if (arrEnemy.length < 15) {
    ...
    arrEnemy.push({ x: tx, y: ty, w: twh, r: tr, h: twh, c: tc, v: tv, hit: 0 });
  }
}
...
function drawEnemy() {
  ...
  for (i = 0; i < arrEnemy.length; i += 1) {
    ...
    if (arrEnemy[i].x >= 300 && arrEnemy[i].x <= 600) {  // 300과 600사이에선 적이 원으로 변함
      ctx.fillStyle = "hsl(" + hue + ", 100%, 50%)";
      ctx.beginPath();
      ctx.arc(arrEnemy[i].x + arrEnemy[i].r, arrEnemy[i].y + arrEnemy[i].r, arrEnemy[i].r, 0, Math.PI * 2);
      ctx.fill();
    } else {
      ctx.fillStyle = "hsl(" + hue + ", 100%, 50%)";
...
function deleteRocket() {
  var i;
  for (i = 0; i < arrRocket.length; i += 1) {  // 방향이 바뀌어도 로켓 개수 감소
    if (tu_chR) {
