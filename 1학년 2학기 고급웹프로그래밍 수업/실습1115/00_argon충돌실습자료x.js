...
function drawShip() {
  ...
  ctx.arc(sx + 5, sy, 15, 0, Math.PI * 2);
  ctx.stroke();

  ctx.beginPath();
  ctx.arc(sx + 30, sy, 10, 0, Math.PI * 2);
  ctx.stroke();

  ctx.strokeStyle = "black";
}

function hitShip_circle() {
  var x0, x1, y0, y1;
  var dx, dy, distance, dx1, dx2, distance2;
  ...
  for (var i = 0; i < arrEnemy.length; i += 1) {
    dx = sx + 5 - (arrEnemy[i].x + arrEnemy[i].wh/2);
    dy = sy - (arrEnemy[i].y + arrEnemy[i].wh/2);
