...
function drawRect() {  // 작은 원 그리기
    ...
    ctx.fillText("R2", rt2.x - 20, rt2.y - 10);
    ctx.fillText("(pt.x, pt.y)", rt2.x - 10, rt2.y + 25);
}
...
function hit_rt_rt(ct, ct2) {  
    ...
    dx = ct.x - ct2.x;
    dy = ct.y - ct2.y;
    distance = Math.sqrt(dx * dx + dy * dy); // 두 원의 중점의 거리 

    if(distance < ct.r + ct2.r) { // 원의 중점거리가 크면 true
        return true;
    } else {
        return false;
    }
